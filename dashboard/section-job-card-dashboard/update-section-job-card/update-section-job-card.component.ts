...
@Component({
  selector: 'update-section-job-card',
  templateUrl: './update-section-job-card.component.html',
  styleUrls: ['./update-section-job-card.component.scss'],
  providers: [
    {
...
export class UpdateSectionJobCardComponent implements OnInit {

  FiledLabels = FieldLabelsConstant.LABELS;
  Titles = FieldLabelsConstant.TITLE;  
  displayedColumns = ['assetType','position','serialNumber','observation','kind','assignedTo','compliance','workDone'];
  historyDisplayedColumns=['sno','updatedOn','updatedBy','changeFrom'];
  inwardLocoList : any;
  ...
  createdBy: any;  
    ...
    assetTypeList:any;
    maxDate = new Date();
    statusValidChangeList:any;
    isOpen: boolean = false;
    jobCardActivityCategoryList: any;
    obsColorCode: any;
    abnormalColorCode: any;
    failuresColorCode: any;
    memosColorCode: any;
    schedulesColorCode: any;
    rapColorCode: any;
  ...
  getJobCardActivityCategory() {
      this.sendAndRequestService.requestForGET(Constants.app_urls.REPORTS.GET_STATUS_ITEM_ADDITIONAL_ATTRIBUTES_DATA_BASED_ON_STATUS_TYPE_ID + Constants.ELS.JOB_CARD_ACTIVITY_CATEGORY).subscribe((data) => {
          ...
          this.jobCardActivityCategoryList.filter(value => {
              if (value.statusItemCode == Constants.ACTIVITY_CATEGORY.OBSERVATIONS) {
                  this.obsColorCode = value.statusItemAttribute1;
              } else if (value.statusItemCode == Constants.ACTIVITY_CATEGORY.ABNORMALITY) {
                  this.abnormalColorCode = value.statusItemAttribute1;
              } else if (value.statusItemCode == Constants.ACTIVITY_CATEGORY.FAILURES) {
                  this.failuresColorCode = value.statusItemAttribute1;
              } else if (value.statusItemCode == Constants.ACTIVITY_CATEGORY.MEMOS) {
                  this.memosColorCode = value.statusItemAttribute1;
              } else if (value.statusItemCode == Constants.ACTIVITY_CATEGORY.SCHEDULE_ITEMS) {
                  this.schedulesColorCode = value.statusItemAttribute1;
              } else {
                  this.rapColorCode = value.statusItemAttribute1;
              }
  ...
  getSectionJobcardData(){
    ...
    .subscribe((resp) => {
      ...
      console.log("getSectionJobcardData="+JSON.stringify(this.sectionJobcardData)  )
      if(this.resp){
      ...
      for(let i = 0;   i < this.sectionJobcardData.length ; i++) {
        ...
        this.dateIssued = this.datePipe.transform(this.sectionJobcardData[i].dateIssued, 'dd-MM-yyyy HH:mm:ss');
         ...
         this.updatedOn = this.datePipe.transform(this.sectionJobcardData[i].updatedOn, 'dd-MM-yyyy HH:mm:ss');
          ...
          this.sendAndRequestService.requestForGET(Constants.app_urls.ENERGY_BILL_PAYMENTS.STATUS_VALID_CHANGE.GET_STATUSIDTO_BASED_ON_STATUS_TYPE_ID_AND_STATUS_ID+'EL_LOCO_JOBCARD_STATUS_TYPES'+'/'+ this.sectionJobcardData[i].status )
          .subscribe((resp) => {
            this.statusValidChangeList = resp;
    console.log("statusvalidlist==="+ JSON.stringify(this.statusValidChangeList))
          });
...
findStatusValidChange(){
  this.sendAndRequestService.requestForGET(Constants.app_urls.ENERGY_BILL_PAYMENTS.STATUS_VALID_CHANGE.GET_STATUSIDTO_BASED_ON_STATUS_TYPE_ID_AND_STATUS_ID+ this.status+'/'+ 'EL_SECTION_JOBCARD_STATUS_TYPES')
      .subscribe((resp) => {
        this.statusValidChangeList = resp;
console.log("statusvalidlist==="+ JSON.stringify(this.statusValidChangeList))
      });
      ...
      prepareDataInObject(arrayObject: any): any {
          arrayObject.remarks = this.remarks;
          arrayObject.targetTime = !!this.targetDate ? new Date(this.targetDate) : '';
          arrayObject.updatedOn = !!this.updatedOn ? new Date(this.updatedOn) : '';
          arrayObject.returnedBy = this.returnedBy;
          arrayObject.status = this.status;
          arrayObject.updatedBy = this.loggedUserData.username;
          return arrayObject;
      }
...
save(){
  ...
  for (var i = 0; i < this.obsArray.length; i++) {
          this.sectionJobs.push(this.prepareDataInObject(this.obsArray[i]));
      }
  for (var i = 0; i < this.abnormalityDataArray.length; i++) {
      this.sectionJobs.push(this.prepareDataInObject(this.abnormalityDataArray[i]));
  }
  for (var i = 0; i < this.insCheckListArray.length; i++) {
      this.sectionJobs.push(this.prepareDataInObject(this.insCheckListArray[i]));
  }
  for (var i = 0; i < this.memoArray.length; i++) {
      this.sectionJobs.push(this.prepareDataInObject(this.memoArray[i]));
  }
  for (var i = 0; i < this.onlineFailuresArray.length; i++) {
      this.sectionJobs.push(this.prepareDataInObject(this.onlineFailuresArray[i]));
  }
...
acknowledge(){  
  ...
  for (var i = 0; i < this.sectionJobcardData.length; i++) {
    ...
    if( this.sectionJobcardData[i].status =='Not Acknowledged/ sent' ){
      this.sectionJobcardData[i].status = "acknowledged" ;
...
checkTab(index: number){
  ...
  case 1:{
    ...
    this.sendAndRequestService.requestForGET(Constants.app_urls.ENERGY_BILL_PAYMENTS.STATUS_HISTORY.GET_STATUS_HISTORY_BY_STATUS_HISTORY_TYPE+'SECTION_JOBCARD').subscribe(response => {
     ...
     // console.log("history list=="+ JSON.stringify(this.historyList)+"jobcardid==="+this.secJobcardId )
      for (let i = 0; i < this.historyList.length; i++) {
      //  console.log("list of section job card id=="+this.historyList[i].sectionJobcardId)        
        if(this.historyList[i].sectionJobcardId == this.secJobcardId )
        {
         ...
         this.historyData.push(this.historyList[i]);
        }
        
      } 
     // console.log("history dataa==="+JSON.stringify(this.historyData)   )
...
AdditionalAct(){ 
    const dialogRef = this.dialog.open(AddAdditionalJobsComponent, {
        height: '400px',
        width: '80%',
        data: {
          sectionJobcardId : this.secJobcardId
              }
      });   
...
@Component({
  selector: 'app-add-additional-jobs',
  templateUrl: 'add-additional-jobs.component.html',
  styleUrls: []
})
export class AddAdditionalJobsComponent implements OnInit {
  additionalJobsFormGroup: FormGroup;
  FiledLabels = FieldLabelsConstant.LABELS;
  Titles = FieldLabelsConstant.TITLE;  
  kindList:any;
  sourceList:any;
...
constructor(
  ...
  private spinnerService: Ng4LoadingSpinnerService,
  private sendAndRequestService: SendAndRequestService,
  public dialogRef: MatDialogRef<AddAdditionalJobsComponent>,
  ...
  if (data) {      
    ...
    this.id=data.secJobcardId;
}
 }

  ngOnInit() {
      this.additionalJobsFormGroup = this.formBuilder.group({
        source: [null],
        kind: [ null],
        observations:[null, Validators.required ]
      });
      this.sendAndRequestService.requestForGET(Constants.app_urls.DRIVE.DRIVE_CHECK_LIST.GET_STATUS_ITEM+'EL_SIZE_CATEGORIES').subscribe((data) =>{
        this.kindList = data;
        this.spinnerService.hide();
          }, error => {
        this.spinnerService.hide();
        })
        this.sendAndRequestService.requestForGET(Constants.app_urls.DRIVE.DRIVE_CHECK_LIST.GET_STATUS_ITEM+'EL_OBSERVATION_TYPES').subscribe((data) =>{
          this.sourceList = data;
          this.spinnerService.hide();
            }, error => {
          this.spinnerService.hide();
          })
          this.sendAndRequestService.requestForGET(Constants.app_urls.SECTION_JOB_CARD.GET_SECTION_JOBCARD_BY_ID+this.id).subscribe((data) =>{
  ...
  saveAdditionalJobs(){
    if (this.title ==  Constants.EVENTS.ADD) {
      let save = {
         'sectionJobcardId':this.sectionJobcardData,
         'source':this.additionalJobsFormGroup.value.source,
         'kind':this.additionalJobsFormGroup.value.kind,
         'actionTaken':this.additionalJobsFormGroup.value.observations,
          'createdBy': this.loggedUserData.username,
           'createdStamp': new Date()
      };

      this.sendAndRequestService.requestForPOST(Constants.app_urls.JOBCARD_ACTIVITY_DETAILS.ADD_JOBCARD_ACTIVITY_DETAILS, save, false).subscribe(response => {
